...
export default function RestaurantMenu({ params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  async function loadCategories() {
    ...
    //   const responseRes = await api.get(`/restaurants/${resolvedParams.id}`);

      setCategories(response.data);
    //   setRestaurantName(responseRes.data.name);
  ...
  async function handleCreateCategory(e: FormEvent) {
    e.preventDefault();
    try {
      await api.post('/categories', { name: newCategoryName, restaurantId: resolvedParams.id });
      setNewCategoryName('');
      loadCategories();
    } catch (error) {
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-8 text-gray-900">
      <div className="max-w-4xl mx-auto">
        ...
        <h1 className="text-3xl font-bold mb-8">Gestão do Cardápio</h1>
        ...
        <div>
          <h2 className="text-xl font-semibold mb-4">Seu Cardápio Atual</h2>
          {categories.length === 0 && <p className="text-gray-500">Nenhuma categoria cadastrada.</p>}
          
          {categories.map((category) => (
            <div key={category.id} className="mb-6">
              <h3 className="font-bold text-lg border-b border-gray-300 pb-2 mb-4">{category.name}</h3>
