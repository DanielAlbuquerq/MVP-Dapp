...
### Regras AWS Servidor:
Como configurar as Inbound Rules para este cenárioVocê deve alterar o Grupo de Segurança (Security Group) do RDS para permitir apenas o IP ou o Grupo de Segurança do seu servidor backend, e não dos usuários finais.Cenário 1: Sua API está hospedada na AWS (EC2, ECS, Lambda)Esta é a forma mais segura. Você não precisa liberar nenhum endereço IP.Vá até as Inbound Rules do seu RDS.Em Source (Origem), selecione Custom.Em vez de digitar um IP, digite o ID do Security Group da sua API (ex: sg-12345678).
